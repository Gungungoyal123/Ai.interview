...
export const startinterview=async(req,res)=>{
    try {
          const { userId, role, experience, techStack, difficulty, companyType, interviewType } = req.body;
...
const conversationhistory=[
  ...
  const response = await groq.chat.completions.create({
    ...
    return res.status(200).json({success:true,sessionid:session._id,firstquestion});
    } catch (error) {
